...
double Potential()
{
    double r2, term, Pot, x, y, z, quot;
    // double rnorm, term1, term2;
    double sigmaP = sigma * sigma;
    int i, j;
    // int k;
    Pot = 0.;
    // Vena: Duplicate results calculated:
    // The order of two points doesn't change the distance between them
    for (i = 0; i < N - 1; i++)
    {
        for (j = i + 1; j < N; j++)
        {
            ...
            // Vena: Smart calculations (speedup 2x)
            quot = sigmaP / r2;
            term = quot * quot * quot * quot * quot * quot;
            Pot += (term - 1) * term;
            /*quot = sigma/sqrt(r2);
            term2 = pow(quot,6.);
            term1 = pow(quot,12.);
    ...
    // Vena: epsilon is equal to 1
    // Vena: Factor out multiplication
    // Vena: Duplicate the distances
    return 8 * Pot;
}

void computeAccelerationsMemory()
{
    int i, j, k;
    double f, rSqd, rCubed, rInvSeven;
    // Vena: Using variables for better register use
    double rx, ry, rz;
...
void computeAccelerations()
{
    ...
    { // loop over all distinct pairs i,j
        for (j = i + 1; j < N; j++)
        {
            ...
            // Vena: Simplify calculations for ILP
            rCubed = rSqd * rSqd * rSqd;
            f = (48 - 24 * rCubed) / (rCubed * rCubed * rSqd);
            // Vena: Simple loop unroll
            a[i][0] += rx * f;
            a[j][0] -= rx * f;
            a[i][1] += ry * f;
